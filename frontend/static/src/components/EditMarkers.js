...
class EditMarkers extends React.Component{
  ...
  render(){
    ...
    const locations = this.state.locations.map(location =><button key={location.id} className='btn location-title'>{location.name}</button>)
    return(
      <React.Fragment>
      <div className='row locations-list-row'>
        <div className='col-8 card locations-list flex-direction-row'>
            {locations}
        </div>
      </div>
      <form className="col-12 col-md-6 mb-5 form" onSubmit={(event)=> this.addLocation(event)}>
